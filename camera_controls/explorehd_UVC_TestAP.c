...
static void usage(const char *argv0)
{
	...
	TestAp_Printf(TESTAP_DBG_USAGE, "explorehd XU supported options:\n");
	...
	TestAp_Printf(TESTAP_DBG_USAGE, "     --xuget-chip              Read explorehd Chip ID\n");
...
// cjc -
	...
	if(ret<0)
		TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Ctrl_ReadChipID Failed\n");
	// cjc -
	...
	if(do_add_xu_ctrl)
	{
		...
		if(ret<0)
		{
			if(ret == -EEXIST)
			{
				TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : Initial XU Ctrls");
			}
			else
			{
				TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : Initial XU Ctrls Failed (%i)\n",ret);
			}
			//TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : ");//Initial XU Ctrls ignored, uvc driver had already supported\n");//\t\t\t No need to Add Extension Unit Ctrls into Driver\n");
	...
	if(do_xu_set_qp)
	{
		if(XU_H264_Set_QP(dev, m_QP_Val) < 0 )
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_H264_Set_QP Failed\n");
	}

	if(do_xu_get_qp)
	{
		if(XU_H264_Get_QP(dev, &m_QP_Val))
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_H264_Get_QP Failed\n");
	}
	...
	if(do_xu_get_br)
	{
		...
		if(m_BitRate < 0 )
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_H264_Get_BitRate Failed\n");
	...
	if(do_xu_set_br)
	{
		if(XU_H264_Set_BitRate(dev, m_BitRate) < 0 )
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_H264_Set_BitRate Failed\n");
	}
	...
	if(do_xu_set)
	{
		...
		if(XU_Set_Cur(dev, SetXU_ID, SetCS, SetCmdDataNum, SetData) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Set Failed\n");
	}
	...
	if(do_xu_get)
	{
		...
		if(XU_Get_Cur(dev, GetXU_ID, GetCS, GetCmdDataNum, GetData) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Get Failed\n");
	}
...
// cjc +	
	if(do_multi_stream_set_type)
	{
        ...
        {
            TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : Multistream format doesn't support(%x)\n", multi_stream_format);
            return 1;
        }
		...
		if(XU_Multi_Set_Type(dev, multi_stream_format) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Set_Type Failed\n");
	}
	...
	if(do_multi_stream_set_bitrate)
	{
		...
		if(XU_Multi_Set_BitRate(dev, streamID_set, MS_bitrate) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Set_Bitrate Failed\n");
	}
	...
	if(do_multi_stream_get_bitrate)
	{
		...
		if(XU_Multi_Get_BitRate(dev, streamID_get, &MS_bitrate) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Get_Bitrate Failed\n");
	}
	...
	if(do_multi_stream_set_qp)
	{
		...
		if(XU_Multi_Set_QP(dev, streamID_set, MS_qp) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Set_QP Failed\n");
	}
	...
	if(do_multi_stream_get_qp)
	{
		...
		if(XU_Multi_Get_QP(dev, streamID_get, &MS_qp) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Get_QP Failed\n");
	}
	...
	if(do_multi_stream_set_H264Mode)
	{
		...
		if(XU_Multi_Set_H264Mode(dev, streamID_set, MS_H264_mode) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Set_H264Mode Failed\n");
	}
	...
	if(do_multi_stream_get_H264Mode)
	{
		...
		if(XU_Multi_Get_H264Mode(dev, streamID_get, &MS_H264_mode) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Get_H264Mode Failed\n");
	}
	...
	if(do_multi_stream_set_sub_gop)
	{
		...
		if(XU_Multi_Set_SubStream_GOP(dev, MS_sub_gop) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Set_SubStream_GOP Failed\n");
	}
	...
	if(do_multi_stream_get_sub_gop)
	{
		...
		if(XU_Multi_Get_SubStream_GOP(dev, &MS_sub_gop) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Get_SubStream_GOP Failed\n");
	}
	...
	if(do_multi_stream_get_status)
	{
		...
		if(XU_Multi_Get_status(dev, &TestInfo) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Get_status Failed\n");
	}
	...
	if(do_multi_stream_get_info)
	{
		...
		if(XU_Multi_Get_Info(dev, &TestInfo) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Get_Info Failed\n");
	}
	...
	if(do_multi_stream_get_enable)
	{
		...
		if(XU_Multi_Get_Enable(dev, &multi_stream_enable) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Get_Enable Failed\n");
	}
	...
	if(do_multi_stream_set_enable)
	{
		...
		if(XU_Multi_Set_Enable(dev, multi_stream_enable) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Set_Enable Failed\n");
	}
	else
	{
	...
	if(do_osd_timer_ctrl_set)
	{
		...
		if(XU_OSD_Timer_Ctrl(dev, osd_timer_count) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Timer_Ctrl Failed\n");
	}
	...
	if(do_osd_rtc_set)
	{
		...
		if(XU_OSD_Set_RTC(dev, osd_rtc_year, osd_rtc_month, osd_rtc_day, osd_rtc_hour, osd_rtc_minute, osd_rtc_second) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Set_RTC Failed\n");
	}
	...
	if(do_osd_rtc_get)
	{
		...
		if(XU_OSD_Get_RTC(dev, &osd_rtc_year, &osd_rtc_month, &osd_rtc_day, &osd_rtc_hour, &osd_rtc_minute, &osd_rtc_second) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Get_RTC Failed\n");
	}
	...
	if(do_osd_size_set)
	{
		...
		if(XU_OSD_Set_Size(dev, osd_size_line, osd_size_block) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Set_Size Failed\n");
	}
	...
	if(do_osd_size_get)
	{
		...
		if(XU_OSD_Get_Size(dev, &osd_size_line, &osd_size_block) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Get_Size Failed\n");
	}
	...
	if(do_osd_color_set)
	{
		...
		if(XU_OSD_Set_Color(dev, osd_color_font, osd_color_border) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Set_Color Failed\n");
	}
	...
	if(do_osd_color_get)
	{
		...
		if(XU_OSD_Get_Color(dev, &osd_color_font, &osd_color_border) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Get_Color Failed\n");
	}
	...
	if(do_osd_show_set)
	{
		...
		if(XU_OSD_Set_Enable(dev, osd_show_line, osd_show_block) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Set_Enable Failed\n");	
	}
	...
	if(do_osd_show_get)
	{
		...
		if(XU_OSD_Get_Enable(dev, &osd_show_line, &osd_show_block) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Get_Enable Failed\n");
	}
	...
	if(do_osd_autoscale_set)
	{
		...
		if(XU_OSD_Set_AutoScale(dev, osd_autoscale_line, osd_autoscale_block) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Set_AutoScale Failed\n");
	}
	...
	if(do_osd_autoscale_get)
	{
		...
		if(XU_OSD_Get_AutoScale(dev, &osd_autoscale_line, &osd_autoscale_block) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Get_AutoScale Failed\n");
	}
	...
	if(do_osd_ms_size_set)
	{
		...
		if(XU_OSD_Set_Multi_Size(dev, osd_ms_size_stream0, osd_ms_size_stream1, osd_ms_size_stream2) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Set_Multi_Size Failed\n");
	}
	...
	if(do_osd_ms_size_get)
	{
		...
		if(XU_OSD_Get_Multi_Size(dev, &osd_ms_size_stream0, &osd_ms_size_stream1, &osd_ms_size_stream2) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Get_Multi_Size Failed\n");
	}
	...
	if(do_osd_position_set)
	{
		...
		if(XU_OSD_Set_Start_Position(dev, osd_type, osd_start_row, osd_start_col) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Set_Start_Position Failed\n");
	}
	...
	if(do_osd_position_get)
	{
		...
		if(XU_OSD_Get_Start_Position(dev, &osd_line_start_row, &osd_line_start_col, &osd_block_start_row, &osd_block_start_col) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_OSD_Get_Start_Position Failed\n");
	}
	...
	if(do_md_mode_set)
	{
		...
		if(XU_MD_Set_Mode(dev, md_mode) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Set_Mode Failed\n");
	}
	...
	if(do_md_mode_get)
	{
		...
		if(XU_MD_Get_Mode(dev, &md_mode) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Get_Mode Failed\n");
	}
	...
	if(do_md_threshold_set)
	{
		...
		if(XU_MD_Set_Threshold(dev, md_threshold) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Set_Threshold Failed\n");
	}
	...
	if(do_md_threshold_get)
	{
		...
		if(XU_MD_Get_Threshold(dev, &md_threshold) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Get_Threshold Failed\n");
	}
	...
	if(do_md_mask_set)
	{
		...
		if(XU_MD_Set_Mask(dev, md_mask) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Set_Mask Failed\n");
	}
	...
	if(do_md_mask_get)
	{
		...
		if(XU_MD_Get_Mask(dev, md_mask) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Get_Mask Failed\n");
	}
	...
	if(do_md_result_set)
	{
		...
		if(XU_MD_Set_RESULT(dev, md_mask) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Set_RESULT Failed\n");
	}

	if(do_mjpg_bitrate_set)
	{
		if(XU_MJPG_Set_Bitrate(dev, mjpg_bitrate) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MJPG_Set_Bitrate Failed\n");
	}

	if(do_mjpg_bitrate_get)
	{
		if(XU_MJPG_Get_Bitrate(dev, &mjpg_bitrate) <0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MJPG_Get_Bitrate Failed\n");
	}
...
// cjc -
	...
	if (do_bri_set) 
	{
		...
		TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : Set Brightness (%d) Failed\n", m_bri_val);
	} 
	...
	if (do_bri_get) 
	{
		...
		if(m_bri_val < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : Get Brightness Failed\n");
		TestAp_Printf(TESTAP_DBG_FLOW, "explorehd_UVC_TestAP @main : Get Brightness (%d)\n", m_bri_val);
	}

	if (do_shrp_set) 
	{
	    if (v4l2SetControl (dev, V4L2_CID_SHARPNESS, m_shrp_val)<0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : Set Sharpness (%d) Failed\n", m_shrp_val);
	} 
	...
	if (do_shrp_get) 
	{
		...
		if(m_shrp_val < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : Get Sharpness Failed\n");
		TestAp_Printf(TESTAP_DBG_FLOW, "explorehd_UVC_TestAP @main : Get Sharpness (%d)\n", m_shrp_val);
	}
	// Standard UVC image properties setting -------------------------------
	...
	if(do_asic_r)
	{
		if(XU_Asic_Read(dev, rAsicAddr, &rAsicData)<0)		
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Asic_Read(0x%x) Failed\n", rAsicAddr);
		else
			TestAp_Printf(TESTAP_DBG_FLOW, "explorehd_UVC_TestAP @main : XU_Asic_Read(0x%x) = 0x%x \n", rAsicAddr, rAsicData);
	}

	if(do_asic_w)
	{
		if(XU_Asic_Write(dev, wAsicAddr, wAsicData)<0 )
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Asic_Write(0x%x) Failed\n",wAsicAddr);
		else
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Asic_Write(0x%x) success\n",wAsicAddr);
	}
	// Houston 2011/10/14 Asic R/W ---
	...
	if(do_sf_read)
	{
		...
		if(buf==NULL || XU_SF_Read(dev, sf_cmd.addr, buf, sf_cmd.length)<0 )
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_SF_Read(0x%x) Failed\n",sf_cmd.addr);
	...
	if(do_i2c_write)
	{
		if(XU_I2C_Write(dev, i2c_cmd.SlaveId, i2c_cmd.addr, i2c_cmd.addr_length, i2c_cmd.data, i2c_cmd.data_length)<0 )
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_I2C_Write(0x%x) Failed\n",sf_cmd.addr);
	}
	...
	if(do_i2c_read)
	{
		...
		if(buf==NULL || XU_I2C_Read(dev, i2c_cmd.SlaveId, i2c_cmd.addr, i2c_cmd.addr_length, i2c_cmd.data, i2c_cmd.data_length)<0 )
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_I2C_Read(0x%x) Failed\n",sf_cmd.addr);
		free(buf);
	}
...
// cjc -
	...
	if(do_multi_stream_set_sub_fr)
	{
		...
		if(XU_Multi_Set_SubStream_FrameRate(dev, MS_sub_fr) < 0)
			TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_Multi_Set_SubStream_FrameRate Failed\n");
	}
...
// cjc -
		...
		if(do_md_result_get)
		{
			if(XU_MD_Get_RESULT(dev, md_mask) <0)
				TestAp_Printf(TESTAP_DBG_ERR, "explorehd_UVC_TestAP @main : XU_MD_Get_RESULT Failed\n");
		}
